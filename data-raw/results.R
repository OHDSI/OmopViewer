...
installed <- deps |>
  purrr::map_chr(\(x) {
    ver <- as.character(utils::packageVersion(x))
    paste0(x, " ", ver)
  })
file <- here::here("extras", "used_packages.txt")
if (file.exists(file)) {
  lastTime <- readLines(con = file)
  run <- !identical(lastTime, installed)
} else {
  run <- TRUE
}

# update if new packages have been updated
if (run) {
  # cdm reference
  ...
  writeLines(text = installed, con = file)
}
