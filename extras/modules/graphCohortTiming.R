...
ui <- function(id) {
  ...
  fluidPage(
    ...
    plotlyOutput(ns("ct_plot")) %>% withSpinner()
  )
}
...
init_server <- function(id, dataset, filter_input) {
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      ...
      updatePickerInput(session, inputId = "ct_plot_facet", choices = choices)
      updatePickerInput(session, inputId = "ct_plot_color", choices = choices)
    })
